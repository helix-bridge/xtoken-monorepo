...
const bridgeOptions: { name: string; logo: string; link: string }[] = [
  { name: "Helix Bridge", logo: "helix-bridge.png", link: "https://helixbridge.app" },
];

export default function ThirdPartyBridge() {
  return (
    <div className="gap-small rounded-large bg-background p-medium flex flex-col">
      {bridgeOptions.map((option) => (
        <a
          key={option.name}
          className="py-medium inline-flex items-center justify-between rounded-xl px-3 transition-colors hover:bg-white/5"
