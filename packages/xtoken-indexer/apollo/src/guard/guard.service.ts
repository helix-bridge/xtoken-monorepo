...
export class GuardService {
  ...
  private generateDataHash(
    ...
    return this.web3.utils.keccak256("0x1901" + domainSeparator.substring(2) + structHash.substring(2));
  }

  private ecrecover(hash: string, sig: string): string {
    const sigObj = ethUtil.fromRpcSig(sig);
    const pubkey = ethUtil.ecrecover(Buffer.from(hash.substr(2), "hex"), sigObj.v, sigObj.r, sigObj.s);
